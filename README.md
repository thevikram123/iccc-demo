...
The command builds `dist-offline/` with local-file friendly routes and relative assets, removes remote font dependencies, inlines the built app into a direct-open `index.html`, bundles `vendor/transformers.min.js`, then writes `offline-build/iccc-demo-offline.zip`. Extract the zip and open its `index.html` in a Chromium-based browser. The app shell, dashboards, GIS screens, FRS flow, alerts, infrastructure views, and local media assets are included in the zip. The first AI Copilot run needs internet to cache `google/gemma-3-270m-it`; subsequent runs use the browser cache and do not call the Cloudflare Worker.
